...
2023-11-09，**OP INHERENT RESOLVE** 任務中的一架 USAF ISR 機在敘利亞執行**TARGET DEVELOPMENT** 任務，從 08:05Z 到 19:33Z 共 11 小時 28 分鐘監控一個 VEO（violent extremist group）的住處與一輛**白色 SUV 帶後掛備胎**。任務即將結束 RTB 過程中，**21:53Z** 飛機觀測到「**1 個 probable HC UAP shaped as a bouncy ball**」從南方接近、降低高度、**安全通過該機**，維持 ~424 KN 速度持續 7 分鐘後出脫離視距。

D 系列首份提供「**UAP 完整 41 欄資料卡**」的 MISREP——shape / state / propulsion / signatures / advanced capabilities / first 與 last coordinates / radius / kinetic altitude 全部填表。
...
| 08:05Z - 15:52Z | **6X stop follow-on PROB VOI + PROB POI**（6 次跟監）|
...
ISR Activity Description: **TARGET DEVELOPMENT**。Global Campaign Plan: **GCP - VEO**（violent extremist group，意指 ISIS 殘餘或敘利亞境內反美武裝團體）。

## UAP 完整資料卡：D 系列首見填表格式

D74 是 D 系列裡**第一份明確填寫 41 欄 UAP 資料卡**的 MISREP，後續同期報告應該都會用這個格式。把所有欄位列出：
...
「PROB HC UAP」這個縮寫 D 系列罕見。**HC** 在 MISREP / SPEAR 命名約定中可能對應 **Hostile Craft**（敵對載具）或 **High-Confidence**（高置信度）。考量 narrative 同時用「PROB」（probable）+「HC」+「UAP」三層描述，意味分析者建議**這是可能來自敵方的高置信度 UAP 觀測**。
...
- [#053 D38](../053-dow_uap_d38_range_fouler_debrief_middle_east_may_2020/report.md)：**ROUND** + ATFLIR solid white（2020-05-14 波斯灣 F/A-18）
- **D74**：**BOUNCY BALL** + 424 KN consistent + drops altitude + 7 min
...
D74 自填表「Physical State: Solid」+「Propulsion: UNK」+「Maneuverability: NONE」與「Advanced Capabilities: YES, 424KN consistently」**形成內部矛盾**：solid 物體保持 424 KN 持續 7 分鐘**不可能沒有推進**。要嘛是 ballistic（重力 + 投擲動能）、要嘛是 propulsion 但 sensor 看不到。

「Drop altitude and safely pass」這個動作意味 **UAP 不是被動風飄**，是**主動降低高度繞過己方飛機**。但表填「Maneuverability Observations: NONE」+「Intelligent Control: NO」。分析者選擇用**保守**的方式記錄，把可觀察到的機動行為歸為「pass」而不是「manoeuver」。

## OP INHERENT RESOLVE 的 VEO 監視脈絡

D74 不是純粹 UAP encounter，是「**反 ISIS / 反 VEO 監視任務的副產品**」。整個 11 小時 28 分鐘任務的主要目標：

- 觀察 white SUV with rear-mounted tire（typical Daesh / VEO 載具特徵）
- 跟監 PROB POI（Person of Interest）+ ADM（Adult Male / armed individual）
- 6 次 stop follow-on（連續跟拍）
- 等待 EEI（Essential Elements of Information）出現

任務結束 RTB 過程中才出現 UAP。這個時間序意味：

1. **UAP 不是任務目標**：sensor 操作員在 RTB 時被 UAP 經過視野觸發觀察
2. **無預警出現**：與其他 D 系列預先 cued by radar / SIGINT 的觀測不同
3. **與 VEO 活動無關**：UAP 從**南方**來、無 emission、無 effects on equipment——看不出與地面 VEO 活動有任何因果關聯

## 解密路徑：MDR 25-0072（比後續批次早）
...
注意 D74 的解密推薦者是 **Tegtmeier**，與 D62 / D63 / D64 / D65 的 **Harrison**（MDR 26-0028 批次）**不同**。MDR 25-0072 是 2025 財年（更早一年），意味 D74 是**較早就送審的案件**，可能因為含完整 41 欄資料卡、UAP signatures 可清楚記錄、無敏感對手關聯，所以較快放行。
...
- [#075 D7 阿拉伯灣 2020](../150-dow_uap_d7_mission_report_arabian_gulf_2020/report.md)：**Balloon-shaped UAP** + 31,000 ft + wind-driven，是 D74「bouncy ball」shape 的前期案例。
- [#053 D38 波斯灣 2020-05-14](../053-dow_uap_d38_range_fouler_debrief_middle_east_may_2020/report.md)：F/A-18 ATFLIR **ROUND** + solid white，圓形 UAP 觀測。
