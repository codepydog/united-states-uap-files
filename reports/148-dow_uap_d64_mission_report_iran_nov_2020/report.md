...
2020-11-02，**482 ATKS / 432 AEW** 的 MQ-9 Reaper 又一次從 OKAS 起飛，20 小時 42 分鐘任務。這次的任務 ATO 是 **EX**（不是 D62/D63 的 DR），主要差別是任務內容增加了一段 **「開放水域掃描，搜尋 UUV」**。在這段水下無人載具搜索期間，**MQ-9 觀測到 2 個 UAP，5 分鐘間隔**。
...
D 系列 70 多份 MISREP 中，**D64 是第一份明確寫出「OPEN WATER SCANS ISO UUV」的任務**。

UUV 是 **Unmanned Underwater Vehicle**（無人水下載具）。MQ-9 從 FL220 高空搜尋 UUV 看起來矛盾，因為 UUV 在水面下，MQ-9 的 EO/IR + ANDAS4 SAR 無法穿透水。但實務上，MQ-9 搜尋 UUV 是看：
...
**2020 年是伊朗 UUV 能力快速發展的時間點**。伊朗 2020 年公佈過 Fateh 級柴電潛艇 + 多型小型 UUV，可能用於水雷布放、情資蒐集、或攻擊型 USV/UUV。MQ-9 在阿拉伯灣專門掃水面的 3 小時段，是 NAVCENT 對抗伊朗潛在水下威脅的偵察行動。
...
**UAP #1** 給出 bearing 080T（東向），altitude unknown。
**UAP #2** 給出 traveling NW（西北向）。

如果 UAP #1 朝東 + UAP #2 朝西北，兩個方向相差 130 度，意味是**兩個不同物體**，不是同一個 UAP 5 分鐘後被重新觀測。這呼應 [#070 D61](../145-dow_uap_d61_mission_report_persian_gulf_aug_2020/report.md) 的 FORMATION + [#047 D3](../047-dow_uap_d3_mission_report_arabian_gulf_2020/report.md) 的 4 個 UAP（含 2 個 side-by-side），D 系列「同戰區同任務段多個 UAP」的累積證據。
...
加上 Additional Avionics 換成 **AH_GMESH_VORTEX**（D62/D63 用 AH-BS_WARIO），意味 D64 的 sensor suite 配置不一樣。AH-BS_WARIO 與 AH_GMESH_VORTEX 都是 MQ-9 的 SIGINT 或通訊中繼模組，**VORTEX 系列**較新（2018 年後部署），具備網狀通訊能力。

這架 MQ-9 看起來是「CONUS reach-back + 增強 SIGINT 載荷 + UUV 搜索目標」的特化配置，與 D62/D63 的 NAVCENT 標準支援任務不同。
...
兩個 UAP 出現在 UUV 搜索段的**最後 25 分鐘**，UUV 沒找到，UAP 卻被連續觀測到 2 次。

可能的關聯：

1. **巧合**：MQ-9 sensor focused 在水面意味著它正在仔細看，所以更容易發現任何空中異常
2. **誘餌或干擾**：如果 UAP 確實是伊朗或他方資產，可能是為了讓 MQ-9 從 UUV 搜索轉移注意力
3. **共生 signature**：UAP 與 UUV 同類別「無人化威脅」，可能來自同一作業
4. **觀測偏差**：D 系列其他 NAVCENT MQ-9 任務也都在掃水面，但只有 D64 同時有 UUV 任務段，所以這是**第一次**這個時間重疊被記錄
...
- [#071 D62 荷莫茲 2020-09-16](../146-dow_uap_d62_mission_report_strait_of_hormuz_sep_2020/report.md)：47 天前同 482 ATKS MQ-9、2 次 EMI 失聯 + 1 UAP。
