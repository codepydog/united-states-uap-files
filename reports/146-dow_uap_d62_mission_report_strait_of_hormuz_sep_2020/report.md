...
2020-09-16，482 ATKS / 432 AEW 的 MQ-9 Reaper 在荷莫茲海峽上空執行又一次 21 小時 NAVCENT 支援任務。任務當中發生了 D 系列中極為罕見的密集事件序列：
...
3 次伊朗喊話 + 2 次共 38 分鐘的原因不明 EMI + UAP，**全部在同一架 MQ-9 同一個任務日內發生**。

## 為什麼 FL040 那麼引人注意

第三次喊話 11:41Z 那次，MQ-9 高度只有 **FL040 = 4,000 英呎**。

正常 MQ-9 Reaper 在戰區 ISR 任務的巡航高度是 **FL180 - FL250**（18,000 - 25,000 英呎）。在 4,000 英呎飛行意味：

1. **任務需要低空特寫**：可能在拍攝伊朗船舶 / 港口 / 機場的高解析影像
2. **被迫下降**：高層雲遮、或 sensor 需要更近距離
3. **受到威脅或干擾**：高高度資料鏈中斷，操作員把無人機壓低尋找視距 backup link

FL040 對 MQ-9 是**極度脆弱**的高度——伊朗的 MANPADS（單兵防空飛彈）、輕型防空砲、空對空飛彈都能輕鬆覆蓋。報告寫「PROFESSIONAL」喊話 + 「STANDARD CALL / STANDARD RESPONSE」意味伊朗沒有開火，但 MQ-9 處於可被攻擊的高度。
...
1.4(g) 用來遮 EMI 詳情，意味**「告訴你 MQ-9 哪個系統被哪個頻率干擾」會洩漏美方對抗能力**。也就是說，這兩次 EMI 不是普通的雷暴雲或太陽閃焰干擾——**很可能是針對性電子戰，洩漏會幫助對手改進攻擊**。
...
「LOST LINK **TO POSSIBLE** [遮蔽]」——「TO POSSIBLE」這個介系詞片語暗示**操作員當下判斷失聯是由某個「可能的」來源造成**，但對應的系統 / 對手名稱被 1.4g 遮掉。
...
如果是選項 1（伊朗電子戰），1.4g 的意義是「告訴你伊朗具備這種能力 = 暴露 US 對抗能力」。如果是選項 2（UAP），1.4g 的意義是「告訴你 UAP 有這種能力 = 暴露 US 知道這個 signature」。

報告選用 1.4g 而不是更通用的 1.4a 遮蔽，**意味遮的是「對抗策略」而不是「事件本身」**。
...
最後一次失聯結束 **2 小時 51 分鐘**後才看到 UAP。**兩個事件並非同步**。

如果 EMI 直接由 UAP 造成，UAP 應該出現在 EMI 期間或剛結束時。但 UAP 出現在 EMI 結束很久之後，這支持「EMI 與 UAP 是獨立事件」的解讀——意味伊朗電子戰 + 偶發 UAP 觀測**剛好疊加在同一個 21 小時任務裡**。

不過從整個 D 系列的 pattern 看（D58 直接電子戰、D62 多次 EMI），**「UAP 環境與 EW 環境共存」確實是 USCENTCOM 戰區的事實**。
...
兩個 ID 連號（412 → 414），意味 9/16 那天 USCENTCOM 戰區還有 ID 413 在某地發生。這暗示 **2020-09-16 在 USCENTCOM 整個戰區有大量電子干擾事件**，不只是 D62 這架 MQ-9。
...
- [#070 D61 阿拉伯灣 2020-08-27](../145-dow_uap_d61_mission_report_persian_gulf_aug_2020/report.md)：20 天前同單位的 UAP FORMATION。
- [#069 D60 阿拉伯灣 2020-08-08](../144-dow_uap_d60_mission_report_persian_gulf_aug_2020/report.md)：1 個月前同單位的 UAP TRANSITTING。
- [#067 D58 NA 2020-10](../142-dow_uap_d58_range_fouler_na_oct_2020/report.md)：1 個月後 F-15E + Noise Jamming 案，**D 系列中另一份明確涉及電子戰的紀錄**。
