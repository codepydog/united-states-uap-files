...
## 為什麼 D5 在高遮蔽下仍揭示 D 系列首份「同任務雙 UAP 事件」

前 58 份 D 系列檔案中：
- **單一觀測時刻多 UAP**：D3（27 秒 4 個）、D19（1 分鐘 3 個）、D20（1 分鐘 10-20 個）、D32（45 分鐘 5 次）、D42（peak 3 個）、D44（1 個）
- **同任務多觀測時刻**：罕見

D5 是**第一個明確記錄「同一份 MISREP 內，相隔 8 小時 49 分鐘的兩次完全獨立 UAP 觀測」的案件**。意味該任務的友軍機（或情監偵單位）在跨時段的 ISR 中**遇到了兩組性質完全不同的 UAP**：
...
「**40 vs. 278 KNOTS**」（7 倍速差）+ 「**等速 vs. 加速轉向**」+ 「**1 個 vs. 2 個**」+ 「**中地中海 vs. 東地中海**」：兩次觀測在每個維度上都不同。**這不是同一物體的延續觀測，而是 D 系列罕見的「跨時段跨地理 + 不同物理特徵」雙 UAP 案件**。
...
**(1) D5 是 D 系列首份「同任務內跨時段雙 UAP 事件」案件**：兩次觀測相隔 8 小時 49 分鐘，物體數量、速度、高度、地理位置、機動模式全部不同。意味該任務（可能為長時程 ISR sortie 或多 sortie 拼接的 MISREP）跨越**兩個獨立 UAP 事件**。
...
**(4) 「40 KNOTS 等速 FL160-170」是 D 系列獨特低速中空層 signature**：對比其他 D 系列案件：
   - D3 阿拉伯灣（4 個 UAP，速度未精確記錄）
   - D27 Gulf of Oman（140 KTS 貼水面）
   - D33 東地中海（80 MPH 貼海面）
   - D35 東地中海（30 MPH 貼海面）
   - **D5 觀測 #1（40 KNOTS at FL160-170 中空層等速）** ← **首次明確「慢速 + 中空層 + 持續等速」signature**
