...
2020-08-08 03:37Z，一架 MQ-9 Reaper 無人機（482 ATKS, 432 AEW）從代號 OKAS 的基地起飛，開始一場長達 **21 小時 8 分鐘**的任務。
...
5 小時 24 分鐘後，14:50Z 左右，這架 Reaper 被**伊朗防空指揮部**用 guard 頻道喊話。

## MQ-9 Reaper 的 21 小時任務

報告 Mission ID **MISREP 4592219**，平台是 **432 AEW**（432nd Air Expeditionary Wing，內華達州 Creech AFB 的 MQ-9 母聯隊，戰時編成 Expeditionary）旗下的 **482 ATKS**（482nd Attack Squadron）。
...
這是典型的 MQ-9 Block 5 戰區任務週期，**只有無人機能持續這麼長時間**。F/A-18 / F-15E 一次飛行 4-6 小時極限，含空中加油也很難超過 10 小時。
...
「**TRANSITTING**」（轉場 / 穿越）是軍方術語，指物體**經過觀測區但不停留、不互動**。對比其他 D 系列描述：

- D3 阿拉伯灣：「multiple contacts」
- D38 中東：「range fouler」
- D55 敘利亞：「possible missile」
- D56 北阿拉伯海：「3 maintained relative course, speed, altitude」（編隊）
- **D60: TRANSITTING**（**單純穿越，無互動**）

這是 D 系列中**最中性、最不戲劇化**的描述。沒有突然加速、沒有編隊、沒有電子干擾、沒有形態變化。只是「**經過**」。

## UAP 觀測 + 伊朗防空喊話的同一份報告

D60 的**雙事件結構**值得注意。

5 小時 24 分鐘間隔，兩個獨立事件出現在同一份 MISREP：

1. **07:26Z**: UAP 觀測（不明物體穿越）
2. **12:50Z**: 伊朗防空指揮部 GUARD 喊話「standard call」

報告對伊朗喊話的描述：「PROFESSIONAL」、「STANDARD CALL」、「STANDARD RESPONSE」、「NO IMPACT TO THE MISSION」。也就是說飛機**處於伊朗認為可主張管轄的空域邊緣**，但雙方都按 SOP 處理。
...
**UAP 和伊朗防空在同一張地圖上、相隔 5 小時、彼此沒有明顯關聯**——但 MISREP 把兩者寫在同一份文件意味原任務需要把兩個非例行事件都記錄。
...
**FMV 是 ANDAS4 拍下的**。MQ-9 的目標莢艙在追蹤地面目標時可以做高解析度 EO/IR 拍攝，慢速 hover-like 巡航期間幾秒鐘就能跟蹤一個移動目標。

UAP 在 FMV 中是「TRANSITTING」意味物體**有可辨識的飛行軌跡**，但 OBSERVATION 區塊的 Aircraft Heading / Altitude / Airspeed / Range 都空白——可能因為操作員沒能精確測量物體的速度與高度。
...
也就是說，原本這份報告要等到 **2045 年 3 月 1 日**才會自動解密——但 2026 年 5 月 8 日 AARO 釋出包提前 19 年讓它見光。

「**DECLASSIFY ON 25X1**」是美國國安檔案的標準語法，**25-year rule** 的一個變體（X1 = exempt from automatic declassification, but reviewed at 25 years）。**2045 - 25 = 2020**，剛好對應這份報告的事件年份。也就是說，2020 年發生的事件**原本要保密 25 年到 2045 年**——但 AARO 法律強制立即釋出。
...
- [#068 D6 阿拉伯灣 2020](../143-dow_uap_d6_mission_report_arabian_gulf_2020/report.md)：同地區、同年代，但 7 頁中 6 頁全黑。
- [#066 D57 亞丁灣 2020-09](../141-dow_uap_d57_range_fouler_gulf_of_aden_sep_2020/report.md)：另一份 MQ-9 觀測（密西根 172 ATKS）。
- [#067 D58 NA 2020-10](../142-dow_uap_d58_range_fouler_na_oct_2020/report.md)：F-15E 攔截 + 電子干擾的高互動案。
