...
2024-07-14，一架 USAF ISR 機在亞丁灣戰區先支援 NAVCENT 後 retask 給 AFCENT。**第一個任務段才執行 1 小時 43 分鐘**，05:17Z 就觀測到 1 個 UAP，**飛機嘗試跟蹤**，但「**UAP'S SPEED WAS FASTER THAN THE [己方平台] FLYING SPEED**」，最後「**FOLLOWED THE UAP TILL THE DISTANCE BECAME TOO FAR TO FOLLOW**」。19 小時 40 分鐘任務裡，**UAP 是最有資訊量的觀測**——其餘時間都是 NAVCENT + AFCENT 標準 target development，narrative 寫 NSTR（Nothing Significant To Report）。

與 [#076 D74](../151-dow_uap_d74_mission_report_syria_nov_2023/report.md) 同樣使用 41 欄 UAP 資料卡格式，但 D75 多了**「己方主動嘗試追蹤」**這個動作，是 D 系列**首份明確記錄飛機主動跟拍但追不上**的案件。
...
| 欄位 | 值 |
|---|---|
| **UAP Maneuverability Observations** | **STRAIGHT FLIGHT PATH AT SAME ALTI**（直線同高度）|
...
兩句拼起來就是事件全貌：**MQ-9 / ISR 機追了 UAP，UAP 速度比 ISR 機快，最後 ISR 機追不上 + UAP 出視距**。
...
1. **UAP > 280 mph 等級**（如果是 MQ-9）
2. **UAP 走 STRAIGHT FLIGHT PATH AT SAME ALTI**（直線同高度）
3. **Kinetic Trajectory: NW**（西北向，與己方 ALL 多向相對）

「ALL flying」+「UAP 走直線」+「UAP 比我快」三項組合提示**這不是被動 wind drift**，UAP 主動維持等速直線 NW，sensor 試圖跟拍但距離拉開。
...
D74 的 UAP 是「被動經過」、D75 是「我方主動追蹤、追不上」。**D75 是 D 系列首份明確記錄 own-aircraft 嘗試 active follow 但被超越的案件**。

## 兩個觀測座標格的地理意義

UAP 首次接觸：**38P MT 53[?]17[?]**
UAP 最後接觸：**38P MT 22[?]49[?]**

38P 是 UTM zone，MT 是 MGRS 100km 格。MT 53→MT 22 對應的位移是 **31 km west + 32 km north ≈ 44 km NW 直線距離**（與 Kinetic Trajectory NW 一致）。

MGRS zone **38P** 涵蓋紅海南端 + 亞丁灣 + 葉門 + 北衣索比亞 / 厄利垂亞 / 索馬利亞蘭。**38P MT 100km 格**對應**葉門中南部到亞丁灣海岸**。標題「Gulf of Aden」對應這個 region 沒問題，但細看 MT 格實際位置可能在**葉門海岸線 inland 端**，不純粹是海上。

ISR 1 為 NAVCENT support，意味第一段 UAP 觀測是支援 **CTF 153 / CMF 紅海護航**或反胡塞武裝任務。亞丁灣 2023-2024 是胡塞攻擊國際航運的高熱區，**CTF 153（Combined Task Force 153 / OPERATION PROSPERITY GUARDIAN）** 由美國海軍主導護航。D75 的 NAVCENT ISR 任務段在這個脈絡中，UAP 出現可能：

1. **胡塞武裝無人機 / 自殺式無人載具**：但這些通常速度 < 200 mph 且 RF emit
2. **伊朗 Shahed-136 / 191 衍生型**：可能但速度約 100-115 mph
3. **未知 high-speed 平台**：D75 narrative 偏向這個解讀
...
兩份在 2025-06-02 同日被推薦解密，意味 USCENTCOM 在那一天**集中處理了一批含 41 欄資料卡的近期 UAP MISREP**。D74（2023）+ D75（2024）兩份都是 SECRET // REL TO USA, FVEY（五眼釋出），這個 caveat 比 D62-D65 的 SECRET // NOFORN 更寬，意味五眼盟國也可看到原文。
...
| **己方反應** | **FOLLOWED THE UAP TILL THE DISTANCE BECAME TOO FAR TO FOLLOW**（D 系列首見此類 active follow 描述）|
| **UAP Physical State** | 1.4g 遮蔽 |
