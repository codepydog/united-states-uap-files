# #081 DOW-UAP-PR21：伊拉克 2022-05，10 秒 IR 影片，兩處對比區並肩移動，疑「可能 SU-27/35」

> **影像限制**：報告描述「兩個對比區並排同進同出」的事件，但原始 mp4 中的 IR 訊號低於可視門檻，11 個候選截幀（每秒一張）皆無法清楚看到兩個獨立熱點。Cover 顯示沙漠／田野背景，視為情境圖；UAP 細節以 DVIDS 官方 caption 與 D 系列對應 MISREP 為準。

![cover](cover.jpg)

PR21 在 14 份 Part 1 PR 影片中是少數有具體飛機型號推測的條目。AARO 將「可能 SU-27/35」的判讀寫入 caption，這個推測來自 D14 MISREP 中影像判讀員的 VISRECCE 紀錄。

## 影片內容

- 長度：10 秒（10.27 秒，DVIDS 鏡像），1920x1080，30 fps
- 10 秒紅外影片。畫面中央同時出現兩處對比區，並肩移動，間距大致固定。兩物體一同進入畫面、一同離開，符合編隊飛行（formation flight）特徵。speed、altitude、heading 均未在 caption 中列出。

![~ t=1.0s 開始](frame-start.jpg)

![~ t=5.1s 中段](cover.jpg)

![~ t=9.2s 結束](frame-end.jpg)

## 對應 D 系列 MISREP

對應 [#037 DOW-UAP-D14](../037-dow_uap_d14_mission_report_eastern_mediterranean_may_2022/report.md)（東地中海 2022-05-29 至 30，MQ-9 OP HUMMER SICKLE 監視俄羅斯地中海艦隊期間，遭 1-3 架敵機通過下方 2 分鐘，其中 1 架疑似 RFAF Su-30）。

D14 同任務 OBS 還包含 2022-05-30 00:11Z 觀測到 1 架可能 Su-27/35 降落於 Bassel Al-Assad，00:29Z 1 架同型起飛南向。PR21 的「並肩兩物體」很可能是這兩個時間點之一的影像，影像判讀員把 thermal/IR signature 比對到 Su-27/35 family。

## 為什麼這份未解

「可能 SU-27/35」是 VISRECCE 推測，不是 IFF 確認。AARO 把它列為 unresolved 的可能原因：

- 沒有 RWR（Radar Warning Receiver）截獲俄方雷達訊號可佐證
- 沒有 transponder ID
- 兩物體的編隊形態與已知俄方部署不完全一致（Bassel Al-Assad 主要部署 Su-24/35）
- 時間點若在 00:11Z 降落觀測，則「並肩移動」可能是視差造成

## 影像規格與來源

| 欄位 | 內容 |
|---|---|
| 系列 | DOW-UAP-PR21 |
| 地點 | Iraq（標籤），實際東地中海 |
| 月份 | 2022-05 |
| 影片長度 | 10 秒 |
| 感測器 | IR（MQ-9 MTS-B） |
| 對應 MISREP | DOW-UAP-D14（[#037](../037-dow_uap_d14_mission_report_eastern_mediterranean_may_2022/report.md)） |
| 公開日 | 2026-05-08 |
| 釋出途徑 | USCENTCOM MDR 25-0094 thru MDR 25-0099 |
| 官方來源 | [DOW-UAP-PR21, Unresolved UAP Report, Iraq, May 2022](https://www.war.gov/UFO/#DOW-UAP-PR21,%20Unresolved%20UAP%20Report,%20Iraq,%20May%202022) |
| DVIDS 鏡像 | [DVIDS video 1006059](https://www.dvidshub.net/video/1006059/) |

DVIDS 鏡像（video ID 1006059）；以下描述依 mp4 截幀與官方 caption 與 D14 MISREP 對應段。

## 相關報告

- [#035 D10 伊拉克 2022-05](../035-dow_uap_d10_mission_report_iraq_may_2022/report.md)，同月份伊拉克任務系列，背景上下文。
- [#036 D12 伊拉克 2022-05](../036-dow_uap_d12_mission_report_iraq_may_2022/report.md)，同月份伊拉克 OP PHANTOM FLEX 觀測。
- [#080 PR20 科威特 2022-05](../080-dow_uap_pr20_image_kuwait_may_2022/report.md)，同月份的單張靜止影像條目，影像格式對照。
